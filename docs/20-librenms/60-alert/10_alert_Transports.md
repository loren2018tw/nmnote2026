...
1. **傳送器**： 設定 librenms 要怎麼傳送訊息（以下示範使用 telegram）以及傳給誰？
2. operations: 設定多久開始傳？傳幾次？傳給誰？
3. **警報範本**： 發生事件時要傳送的內容範本
4. **警報規則**： 什麼事件（條件）會觸發警報


以上步驟必須依次設定，例如一定要先有傳送器， operation 才能設定用什麼傳。警報範本則決定要傳送什麼內容。警報規則發生時，要選擇進行什麼操作 (operations)跟使用什麼範本。
